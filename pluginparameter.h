...
*/
	void setMeterValue(const double& meterValue, const boundVariableType& type)
	{
		if (type == boundVariableType::kFloat)
			*boundVariableFloat = static_cast<float>(meterValue);
		if (type == boundVariableType::kDouble)
			*boundVariableDouble = static_cast<double>(meterValue);
		if (type == boundVariableType::kInt)
			*boundVariableInt = static_cast<int>(meterValue);
